...
class AlbumTest < ActiveSupport::TestCase
  ...
  test "add new song to DIVIDE album" do
    assert_equal 1, Album.where("name = ?", "DIVIDE").joins(:songs).count
    AccessPatterns.addNewSongToDivideAlbum
    assert_equal 2, Album.where("name = ?", "DIVIDE").joins(:songs).count
  end

  test "delete song Islandary updates album song count" do
  ...
  test "update genre of album Wait for it EP" do
    assert_equal "Deep House", Album.where("name = ?", "Wait for it EP").limit(1).pluck(:genre).first
    AccessPatterns.updateGenreOfAlbumWaitForItEP
